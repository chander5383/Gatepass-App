...
function normalizeText(s=''){ return String(s||'').trim().replace(/\s+/g,' ').split(' ').map(w=>{ return w ? (w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()) : ''; }).join(' '); }
...
function buildQrString(){
  ...
  const mrEnabled = printMrPrefixCheckbox && printMrPrefixCheckbox.checked;
  const authRaw = document.getElementById('authPerson').value || '';
  const auth = mrEnabled ? (/^mr\\.?/i.test(authRaw)?authRaw:'Mr. '+authRaw) : authRaw;
